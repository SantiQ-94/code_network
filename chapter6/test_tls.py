	...
	if cert is None:
		...
		if (not server) and names:
			try:
				...
				message = 'Yes'
				say('Whether names match the hostname', message)
...
class PySSLSocket(ctypes.Structure):	
	#Actually i have no idea of what this is supposed to do			
...
def SSL_get_version(ssl_sock):
	"""It should retrieve the SSL socket's version"""
...
def lookup(prefix, name):
	...
	try:
		...
		message = 'Error: {!r} is not one of the available names:\n {}'.format(
				  name, ' '.join(sorted(mathching_names)))
