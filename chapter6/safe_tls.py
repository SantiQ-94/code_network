...
def client(host, port, cafile=None):
	...
	print('Connected to host {!r} and port {!r}'.format(host, port))
...
def server(host, port, certfile, cafile=None):
	...
	listener.bin((host, port))
	listener.listen(1)
	print('Listening at interface {!r} and port {!r}'.format(host, port))
	raw_sock, address = listener.accept()
	print('Connection from host {!r} and port {!r}'.format(*address))
